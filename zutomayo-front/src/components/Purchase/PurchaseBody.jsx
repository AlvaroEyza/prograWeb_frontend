...
const PurchaseBody = () => {
    ...
    useEffect(() => {
        if (userId && userId > 0)
        {
            fetch(`http://localhost:4001/usuario/${userId}`)
    ...
    const fetchItemsCarrito = async () => {
        const response = await fetch('http://localhost:4001/carrito');
    ...
    const actualizarStock = async (carrito) => {
        ...
        for (const index in items) {
            ...
            try {
                const result = await fetch(`http://localhost:4001/music_movie/${item.id}`, payload);
                if (!result.ok) {
    ...
    const handleDeleteCarrito = async () => {
        ...
        const result = await fetch(`http://localhost:4001/carrito`, payload);
    ...
    const handleSubmit = async () => {
        ...
        const result = await fetch('http://localhost:4001/usuario',payload);
    ...
    const handlePurchase = async (carrito) => {
        ...
        const result = await fetch(`http://localhost:4001/orden`, payload);
    ...
    return (
        ...
        <div className='Purchasebody'>
            ...
            <br/><br/>
            <button className='return' type='button' onClick={() => navigate('/cart')}>return</button>
